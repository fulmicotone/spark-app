package com.fulmicotone.spark.java.app.business.listeners;


import com.fulmicotone.spark.java.app.model.IntBox;
import com.fulmicotone.spark.java.app.model.StringBox;
import com.fulmicotone.spark.java.app.model.TypeBox;
import com.fulmicotone.spark.java.app.utils.Constants;
...
public class MyJobProgressListener extends SparkListener {

  public static class JobId extends IntBox{  public JobId(Integer integer) { super(integer); } }

  public static class JobGroupId extends StringBox {  public JobGroupId(String s) {  super(s); }}

  public static class StageId extends IntBox {  public StageId(Integer s) {  super(s); }}

  public static class StageAttemptId extends IntBox{  public StageAttemptId(Integer integer) { super(integer); } }

  public static class PoolName extends StringBox {  public PoolName(String s) {  super(s); }}

  public static class ExecutorId extends StringBox {  public ExecutorId(String s) {  super(s); }}
  ...
  private final String no_group_key="ng";

  // Jobs:
  Map<JobId,JobUIData> activeJobs = new HashMap();
  List completedJobs = new ArrayList<JobUIData>();
  List failedJobs = new ArrayList<JobUIData>();
  Map<JobId,JobUIData> jobIdToData = new HashMap<>();
  Map <JobGroupId,List<JobId>>jobGroupToJobIds = new HashMap();

  // Stages:
  Map<StageId,StageInfo> pendingStages =  new HashMap<>();
  Map<StageId,StageInfo> activeStages =  new HashMap<>();
  List completedStages = new<StageInfo> ArrayList();
  List skippedStages = new<StageInfo> ArrayList();
  List failedStages = new<StageInfo> ArrayList();
  Map<StageId,Map<StageAttemptId,StageUIData>> stageIdToData = new HashMap<>();

  Map <StageId,StageInfo> stageIdToInfo = new HashMap<>();
  Map<StageId,List<JobId>> stageIdToActiveJobIds = new HashMap<>();
  Map<PoolName,Map<Stage,StageInfo>> poolToActiveStages = new HashMap<>();
  ...
  AtomicInteger numCompletedStages = new AtomicInteger(0);
  AtomicInteger numFailedStages = new AtomicInteger(0);
  AtomicInteger numCompletedJobs = new AtomicInteger(0);
  AtomicInteger numFailedJobs = new AtomicInteger(0);

  // Misc:
  Map<ExecutorId, BlockManagerId> executorIdToBlockManagerId = new HashMap<>();

  List blockManagerIds = new ArrayList(executorIdToBlockManagerId.values());


  Optional<SchedulingMode> schedulingMode;
  ...
  /* todo private[spark] def getSizesOfActiveStateTrackingCollections: Map[String, Int] = {
    Map(
  ...
  /* todo private[spark] def getSizesOfHardSizeLimitedCollections: Map[String, Int] = {
    Map(
 ...
 /* todo private[spark] def getSizesOfSoftSizeLimitedCollections: Map[String, Int] = {
    Map(
  ...
  /** If stages is too large, remove and garbage collect old stages todo */
 /* private def trimStagesIfNecessary(stages: ListBuffer[StageInfo]) = synchronized {
  ...
  /** If jobs is too large, remove and garbage collect old jobs todo */
  ...
  public void onJobStart(SparkListenerJobStart jobStart) {
    synchronized (jobStart){


      Optional<String> optJobGroup = Optional
              .ofNullable(jobStart.properties())
              .map(props -> props.getProperty(Constants.Spark.SPARK_JOB_GROUP_ID));


        UIData.JobUIData jobData = new UIData.JobUIData(jobStart.jobId(),
                Option.apply(jobStart.time()),
                Option.empty(),
                jobStart.stageIds(), optJobGroup.map(Option::apply)
                .orElse(Option.empty())
                ,
      ...
      JobGroupId f = new JobGroupId
              (optJobGroup.orElse(no_group_key));


      List<JobId> jobids;

        Optional.ofNullable(jobGroupToJobIds.putIfAbsent(f, (jobids=new ArrayList<>())))
                .orElse(jobids)
                .add(new JobId(jobStart.jobId()));



        JavaConversions.seqAsJavaList(jobStart.stageInfos())
                .forEach(p->pendingStages.putIfAbsent(new StageId(p.stageId()),p ));
